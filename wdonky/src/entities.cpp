...
Entity::Entity(int x, int y, int** grid) 
  : x(x)
  , y(y)
  , grid(grid)
  , frame(0)
...
// coordinate in pixel dell'entità e puntatore della matrice su cui deve muoversi
int Entity::getX() { 
  return x;
}
...
Kong::Kong(int** c, float d) 
  : Entity(80, 60, c)
  , difficulty(d)
  , throwBarrel(81 * d)
{};

void Kong::resetFrame() { 
  frame = 1;
}
...
int Kong::getThrowBarrel() const { 
  return throwBarrel;
}

float Kong::getDifficulty() const { 
  return difficulty;
}
